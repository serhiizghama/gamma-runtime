...
Only use external task trackers (ClickUp, etc.) if the user **explicitly** asks about them by name.`);
    ...
    if (isLeader) {
      ...
      sections.push(this.workerAppPolicy());
...
\`\`\``;
  ...
  private workerAppPolicy(): string {
    return `## Team App — OFF LIMITS

The team has a user-facing HTML application at \`project/app/\` (\`index.html\`, \`style.css\`, \`script.js\`, \`data.json\`). This app is owned exclusively by the **team leader**.

### Hard rules
- **Do NOT create, edit, rename, move, or delete ANY file inside \`project/app/\`.** This includes \`index.html\`, \`style.css\`, \`script.js\`, \`data.json\`, and anything else under that directory.
- This applies even if you think the app has a bug, is missing your data, or would look better with a tweak. Touching the app has previously broken it — you do not have permission to edit it.
- \`project/app/data.json\` is regenerated by the backend on every view from your pipeline artifacts. Writing it has no effect — your edit is discarded.

### Keep writing your contracted outputs
Your Output Contract still stands. You SHOULD keep writing raw-data artifacts to the paths assigned to you (e.g. \`project/detailed-scoring.json\`, \`project/applications/*.md\`, \`project/reports/*.md\`, \`project/vacancies-*.json\`). The app picks those up automatically — that is how your work reaches the user.

### If you genuinely believe the app needs a change
Do NOT edit it yourself. Instead, pick one:
1. **Ask the team leader** via \`send-message\` — describe what you think should change and why. The leader decides and either assigns a dedicated app-edit task or handles it themselves.
2. **Report it as a blocker** via \`report-status\` so the user sees it.

Only proceed to edit \`project/app/\` files if you receive an explicit task assigned to you whose description clearly authorizes editing the app (e.g. "Update the dashboard UI to show X"). A general task is not permission.`;
  }
...
### Ownership & worker permissions
The app is YOUR responsibility. Workers (all non-leader agents) are explicitly forbidden from touching \`project/app/\` and have been told to ask you first. Handle their requests like this:

- If a worker sends you a message asking to edit the app, decide: **approve and assign a dedicated app-edit task to them** (the task description must clearly authorize the edit), **do it yourself**, or **reject** and tell them to keep their output in their contracted artifact files.
- A general task assigned to a worker is NOT permission to edit the app — only a task whose description explicitly says "edit/update the app" counts.
- Workers' raw-data outputs (e.g. \`project/detailed-scoring.json\`, \`project/applications/\`, \`project/reports/\`) are THEIR contracts and must not be moved into \`project/app/\`. The backend composes \`data.json\` from those artifacts automatically.`;
  }
