...
export function TeamMap({ leader, members, onAgentClick }: Props) {
  return (
    <div className="flex h-full flex-col">
      ...
      {leader && (
        <div className="mb-2">
          <AgentNode agent={leader} onClick={() => onAgentClick?.(leader)} />
        </div>
      )}
      ...
      {members.length > 0 && leader && (
        <div className="ml-4 border-l border-gray-700 pl-3 space-y-2">
          {members.map((agent) => (
            <AgentNode key={agent.id} agent={agent} onClick={() => onAgentClick?.(agent)} />
          ))}
        </div>
      )}
